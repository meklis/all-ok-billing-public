...
class ShareDevice extends OmoAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        if(isset($req['phone']) && isset($req['agreement_id']) && isset($req['device_id'])) {
            ...
            $userId = $this->control->addPhone($req['phone'], $req['agreement_id']);
            $this->logger->info("Defined user with id $userId by phone {$req['phone']}");
            $resp = $this->control->shareDevice($userId, $req['device_id']);
