...
function generSMS($a) {
$message  = "Нагадуємо що 1 числа знімається оплата по тарифу. Поповніть  рахунок - ${a['need_to_pay']}грн.";
...
GROUP BY c.id 
HAVING min(cast(ifnull(p.disable_day_static, p.disable_day) as date)) = CURDATE() + INTERVAL 1 DAY and CURDATE() + INTERVAL 1 DAY = CAST(first_date(CURDATE() + INTERVAL 1 DAY) as date)");
