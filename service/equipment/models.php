<?php
$rank = 20;
$table = "<center><h3>По Вашему запросу моделей не найдено</h3></center>";
require($_SERVER['DOCUMENT_ROOT'] . "/include/load.php");
init();

if(!\envPHP\service\PSC::isPermitted('eq_models')) {
    pageNotPermittedAction();
}
if(isset($_REQUEST)) foreach($_REQUEST as $k=>$v) $form[$k]=$v;

if(isset($form['del'])) {
   $test =  $sql->query("DELETE FROM equipment_models WHERE id = %1", $form['del']);
   if($test) {
       $html->addNoty('success', "Модель успешно удалена!");
   } else {
       $html->addNoty('error', "Ошибки при удалении модели: ".$sql->error."");
   }
}
...
if(isset($form['save'])) {
    ...
    foreach ($form['ch'] as $k=>$v) {
       if($v['name'] != '') $test =  $sql->query("INSERT INTO equipment_models (id, name, description) VALUES (%1,%2, %3) ON DUPLICATE key UPDATE name = %2, description = %3", $k, $v['name'], $v['description']); 
        if(!$test) $error = true;
    }
        if($error) {
...
$data = $sql->query("SELECT id,  name, description,  (SELECT count(*) FROM equipment WHERE `model` = s.id) count FROM equipment_models s ORDER BY 2");
...
$max = $sql->query("SELECT max(id)+1 id FROM equipment_models")->fetch_assoc()['id'];
$table .= "<tr><td style='width: 40px'><input class='form-control' name='ch[$max][name]' ><td><input class='form-control'  style = 'width: 300px; ' name='ch[{$max}][description]'><td></td><td></td></table>";
