...
class bindings
{
    ...
    static function add($activationId, $ip, $mac, $switch, $port, $employeeId, $realIp = false, $allow_static = false)
    {
        ...
        if(self::bindDB()->isMacDuplicated($mac, $ip)) {
            throw new \InvalidArgumentException("MAC address is duplicated in VLAN");
        }
        try {
    ...
    static function edit($bindId, $newIp, $newMac, $newSwitch, $newPort, $employeeId, $allow_static = false)
    {
        dbConnPDO()->beginTransaction();
        try {
            $binding = self::bindDB()->getBinding($bindId);
            if($newIp || $newMac) {
                if (self::bindDB()->isMacDuplicated($newMac ? $newMac : $binding['mac'], $newIp ? $newIp : $binding['ip'])) {
                    throw new \InvalidArgumentException("MAC address is duplicated in VLAN");
                }
            }
            $device = new bindingsDevice($bindId);
