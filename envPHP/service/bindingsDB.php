...
class bindingsDB
{
    ...
    public static function getBinding($id = "", $ip = "", $mac = "", $switch = "", $port = "")
    {
        ...
        $resp= $data->fetch();
        $resp['allow_static'] = $resp['allow_static'] ? true : false;
        return $resp;
    }
    ...
    public static function isMacDuplicated($mac, $userIp) {
        $psth = dbConnPDO()->prepare("SELECT b.id
FROM eq_bindings b 
JOIN eq_neth nt on INET_ATON(b.ip) BETWEEN INET_ATON(nt.startIp) and  INET_ATON(nt.stopIp)
WHERE b.mac = ? and (INET_ATON(?) BETWEEN INET_ATON(nt.startIp) and INET_ATON(nt.stopIp))");
        $psth->execute([$mac, $userIp]);
    ...
    public static function findBindingIp($userMac, $vlanId = 0, $deviceMac = "", $devicePort = 0) {
        ...
        $info = dbConnPDO()->prepare("SELECT DISTINCT b.ip user_ip, b.id, b.mac user_mac
                FROM eq_bindings b 
                ...
                WHERE p.time_stop is null $WHERE");
