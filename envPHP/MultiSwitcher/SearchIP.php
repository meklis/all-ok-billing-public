...
class SearchIP
{
    ...
    private function getFDB($vlan_id, $mac_addr, $ignoreTagged = true) {
        foreach ($this->getSearchDeviceList() as $dev=>$access) {
            ...
            try {
                ...
                if(in_array('fdb', $core->getModulesData()))
            ...
            foreach ($fdb as $f) {
                if($ignoreTagged) {
                    if(in_array($f['interface'], $vlan['ports']['tagged'])) {
                        continue;
                    }
    ...
    private function getCoreInstance($ip, $access) {
        ...
        $core = $connector->getOrInit(
            Device::init($ip, $access['community'], $access['login'], $access['password'])
                ->set('telnetPort', $access['port_telnet'])
                ->set('mikrotikApiPort',$access['port_api'] )
